    ...
    <nav className="fixed top-0 w-full bg-white/80 backdrop-blur-md z-[100] border-b border-slate-100">
      ...
      {isOpen && (
        <motion.div 
          ...
          className="absolute top-20 left-0 w-full bg-white border-b border-slate-100 p-6 flex flex-col gap-6 md:hidden shadow-xl"
        >
          <a href="#tentang" onClick={() => setIsOpen(false)} className="font-bold text-slate-700">Tentang Desa</a>
          <a href="#pelayanan" onClick={() => setIsOpen(false)} className="font-bold text-slate-700">Layanan Publik</a>
          <a href="#lokasi" onClick={() => setIsOpen(false)} className="font-bold text-slate-700">Lokasi Balai Desa</a>
          <button className="w-full py-4 bg-blue-900 text-white rounded-2xl font-bold">
            Hubungi Kami
          </button>
        </motion.div>
      )}
