...
const FaqItem = ({ question, answer }: { question: string; answer: string }) => {
  ...
  return (
    <div className="border-b border-slate-100 last:border-0">
      ...
      >
        <span className={`font-bold transition-colors ${isOpen ? 'text-blue-900' : 'text-slate-700 group-hover:text-blue-900'}`}>
          {question}
        </span>
        <motion.div
    ...
    <section id="faq" className="py-24 bg-white px-6">
      <div className="max-w-3xl mx-auto">
        <div className="flex flex-col items-center text-center mb-16">
          <div className="p-3 bg-blue-50 text-blue-900 rounded-2xl mb-4">
            <HelpCircle size={28} />
          </div>
          <h2 className="text-3xl font-black text-slate-900 mb-4 tracking-tight">Tanya Jawab (FAQ)</h2>
          <p className="text-slate-500">Informasi lengkap seputar prosedur administrasi di Desa Balapulang.</p>
        </div>

        <div className="bg-[#F8FAFC] rounded-[2.5rem] p-8 md:p-12 border border-slate-100 shadow-sm">
          {faqs.map((faq, index) => (
            <FaqItem key={index} question={faq.question} answer={faq.answer} />
          ))}
        </div>
        
        <div className="mt-12 p-6 bg-blue-900 rounded-3xl text-center text-white">
          <p className="text-sm opacity-80 mb-2">Punya pertanyaan lain?</p>
          <p className="font-bold">Hubungi WhatsApp Layanan Desa: (0283) 465123</p>
        </div>
      </div>
    </section>
  );
